...
class Pokemon(commands.Cog):
    ...
    async def _do_catch(self, gid, uid, e, ball: str, one_shot: bool = False):
        # Rework phase 3: thin compatibility shim — the attempt lives in
        # services.catch_service; render/layout stays here for now.
        from services.catch_service import attempt_catch
        return await attempt_catch(gid, uid, e, ball, one_shot=one_shot,
                                   progress_fn=self._catch_progress,
                                   meta_fn=self._catch_meta)
