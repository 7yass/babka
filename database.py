...
Local-first: one plain SQLite file (WAL mode). Daily snapshots go to backups/.
"""
import functools
import sqlite3
...
def init_db():
    with conn_ctx() as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS pk_learnsets (
            dex INTEGER PRIMARY KEY, pool TEXT DEFAULT '[]')''')
        ...
        for col in ('ALTER TABLE eco ADD COLUMN energy INTEGER DEFAULT 100',
                    ...
                    "ALTER TABLE pk_mons ADD COLUMN evs TEXT DEFAULT ''",
                    "ALTER TABLE pk_mons ADD COLUMN held TEXT DEFAULT ''",
                    "ALTER TABLE pk_market ADD COLUMN ivs TEXT DEFAULT ''",
                    "ALTER TABLE pk_market ADD COLUMN evs TEXT DEFAULT ''",
